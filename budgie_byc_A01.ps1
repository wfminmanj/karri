<################### budgie #####################
 ...
 Load 2016 EDL Forms into an MS Access Database
#################################################
...
$P | Add-Member –M NoteProperty –Name bpath   –Val 'c:\tmp\budgie\G\APR25'
$P | Add-Member –M NoteProperty –Name dbname  –Val '2016_EDL_Database_G_asofMAY11.accdb'
$P | Add-Member –M NoteProperty –Name tblf    –Val 'EDL_Form'
$P | Add-Member –M NoteProperty –Name tbli    –Val 'EDL_Item'
$P | Add-Member –M NoteProperty –Name fzip    –Val 'budgie_in2016MAY11.zip'
...
ForEach ($i in (Get-ChildItem $P.ffpath)) {
...
, Digit_Pass, Product_Family, Brand_Name, Item_Description
, Case_Pack, Item_Size, Item_UoM
, Case_List_Cost, Case_Freight, FOB_Flag
, Program_Type
, PCT_OFF, SB_AMT, REG_SRP, EDL_SRP
, MD5_Checksum)
Values ( '$($r.cYear)', '$($r.cUPC)', '$($r.cFr_Date)', '$($r.cTo_Date)', $( $dflag )
, '$($r.cFamily)', '$($r.cBrand -replace [Char]39, ([Char]39 + [Char]39))', '$($r.cItem_Desc -replace [Char]39, ([Char]39 + [Char]39))'
, $($r.cCase_Pk), $($Sz), '$($r.cUoM)'
, $($r.cList_Cost)
, $( If (("X" + $r.cCase_Freight.ToString()).Length -gt 1) { $r.cCase_Freight } Else { 0 } )
, '$($r.cFOB_Flag)'
, '$($r.cProgram_Type)'
, $( If (("X" + $r.cPct_Off.ToString()).Length -gt 1) { $r.cPct_Off } Else { 0 } )
, $( If (("X" + $r.cSB_AMT.ToString()).Length -gt 1) { $r.cSB_AMT } Else { 0 } )
, $([Decimal]$r.cREG_SRP.Trim([char]36)), $([Decimal]$r.cEDL_SRP.Trim([char]36))
, '$($fmd5)'
)
"@
#$in5edl
